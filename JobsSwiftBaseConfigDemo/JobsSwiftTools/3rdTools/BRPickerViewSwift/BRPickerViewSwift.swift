...
public extension Collection { subscript(safe i: Index) -> Element? { indices.contains(i) ? self[i] : nil } }
public extension Comparable { func clamped(_ r: ClosedRange<Self>) -> Self { min(max(self, r.lowerBound), r.upperBound) } }
// 级联数据节点（示例）
