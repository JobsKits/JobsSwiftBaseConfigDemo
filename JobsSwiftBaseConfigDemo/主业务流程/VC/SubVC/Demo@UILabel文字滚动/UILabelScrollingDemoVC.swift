...
final class UILabelScrollingDemoVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 17.0, tvOS 17.0, *) {
            _ = registerForTraitChanges([UITraitUserInterfaceStyle.self]) {
                (controller: UILabelScrollingDemoVC, _) in
...
private extension UILabelScrollingDemoVC {
    func refreshTheme() {
        view.byBackgroundColor(JobsCor.systemGroupedBackground)
        introLabel.byTextColor(JobsCor.secondaryLabel)
        continuousTitleLabel.byTextColor(JobsCor.secondaryLabel)
        pingPongTitleLabel.byTextColor(JobsCor.secondaryLabel)
        shortTitleLabel.byTextColor(JobsCor.secondaryLabel)
        continuousCardView.byBackgroundColor(JobsCor.secondarySystemGroupedBackground)
        pingPongCardView.byBackgroundColor(JobsCor.secondarySystemGroupedBackground)
        shortCardView.byBackgroundColor(JobsCor.secondarySystemGroupedBackground)
        continuousLabel
            .byTextColor(JobsCor.label)
            .byReloadTextScroll()
        pingPongLabel
    ...
    func switchText() {
        ...
        : "UILabel+Scrolling：从标题开头完整滚到结尾，再以固定间距无缝接回开头。"
        continuousLabel
