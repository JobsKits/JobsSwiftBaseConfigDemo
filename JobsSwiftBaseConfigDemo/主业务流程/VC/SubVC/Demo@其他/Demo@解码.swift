...
extension SafeCodableDemoVC {
    ...
    private func useDirtyJSONAndDecode() {
        jsonTextView.byText(dirtyJSONString)
        decodeCurrentJSON()
    }

    private func useCleanJSONAndDecode() {
        jsonTextView.byText(cleanJSONString)
        decodeCurrentJSON()
    }
    ...
    private func clearValues() {
        [idLabel,
         ...
         createdAtLabel,
         homepageLabel,
         avatarURLLabel].forEach { $0.byText("") }
    }
    ...
    private func clearLog() { logTextView.byText("") }
}
