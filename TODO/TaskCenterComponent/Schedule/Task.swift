...
open class STTask {
    ...
    public typealias Action = (STTask) -> Void
    ...
    open func executeNow() {
        let actions = _lock.withLock { () -> Bag<STTask.Action> in
            let now = Date()
    ...
    @discardableResult
    public func addAction(_ action: @escaping (STTask) -> Void) -> ActionKey {
        return _lock.withLock {
...
extension STTask: Hashable {
    ...
    public static func == (lhs: STTask, rhs: STTask) -> Bool {
        return lhs === rhs
    }
}
