...
- 全局主题状态由 `UIApplication.jobsGlobalDarkModeEnabled` 持久化；`jobsToggleGlobalTheme()` 切换明暗模式，`jobsApplyGlobalTheme()` 把当前模式同步到所有已连接 Scene 的 Window；切换完成后发布 `.JobsGlobalThemeDidChange`，供固定色板、列表内容和自绘页面即时刷新。
