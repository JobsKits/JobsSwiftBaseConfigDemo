...
private extension JobsLabelScrollController {
    ...
    func renderedAttributedText() -> NSAttributedString? {
        if let sourceAttributedText {
            ...
            missingColorRanges.forEach { result.addAttribute(.foregroundColor, value: sourceTextColor, range: $0) };return result
        }
        ...
        if let sourceShadowColor, let label {
            let shadow = NSShadow()
            shadow.shadowColor = sourceShadowColor
    ...
    func concealSourceText() {
        ...
        label.attributedText = concealedText
        isConcealed = true
    ...
    func revealSourceText() {
        ...
        label.textColor = sourceTextColor
        label.shadowColor = sourceShadowColor
        if let sourceAttributedText {
            label.attributedText = sourceAttributedText
        } else {
            label.attributedText = nil
            label.text = sourcePlainText
        }
    ...
    func requireMainThread() {
        precondition(Thread.isMainThread, "JobsScrollingLabel must be configured on the main thread.")
    }
}
